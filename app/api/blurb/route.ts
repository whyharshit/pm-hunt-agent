...
import { writeBlurb } from '@/lib/blurb';
...
async function handle(request: Request) {
  ...
  if (tracked.tier === 'green') {
    return Response.json({ ok: false, error: 'green tier — no JD to write blurb from' }, { status: 400 });
  }

  if (!force) {
    const existing = await getBlurb(id);
    if (existing) return Response.json({ ok: true, cached: true, blurb: existing });
  }
  ...
  const tailored = await getTailored(id);

  try {
    const blurb = await writeBlurb(id, jd, tailored);
    await saveBlurb(id, blurb);
    return Response.json({ ok: true, cached: false, blurb });
  } catch (e) {
    return Response.json(
      { ok: false, stage: 'blurb', error: (e as Error).message },
      { status: 500 }
    );
  }
}
